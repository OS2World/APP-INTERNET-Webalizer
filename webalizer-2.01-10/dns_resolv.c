...
while(trav)
{
	if((res_ent = gethostbyaddr((char *)&trav->addr.s_addr,
			sizeof(trav->addr.s_addr),
			AF_INET)))
	{
		...
		if(memcmp(res_ent->h_name, &(trav->addr.s_addr),
			sizeof(trav->addr.s_addr)))
		{
			if(debug_mode)
				printf("Got a result: %s -> %s\n",
					trav->string, res_ent->h_name);
			db_put(trav->string, res_ent->h_name, 0);
		}
		else
		{
			if(debug_mode)
